...
const SkillsSection = () => {
  ...
  return (
    <section ref={sectionRef} id="skills" className="py-16 md:py-24 lg:py-32 relative overflow-hidden">
      {/* Background Gradient */}
      ...
      <div className="container mx-auto px-4 md:px-6 lg:px-8 relative z-10">
        ...
        <div className="grid lg:grid-cols-2 gap-12 md:gap-16 lg:gap-20 items-start">
          ...
          <div className="animate-fade-in-up" style={{ animationDelay: "0.2s" }}>
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 md:gap-6">
              {skillCards.map((skill, index) => (
                <div
                  key={skill.name}
                  className="group bg-card border border-card-border rounded-xl p-6 md:p-8 text-center hover:border-accent/30 transition-all duration-300 hover-glow animate-scale-in"
                  style={{ animationDelay: `${0.1 * index}s` }}
                >
                  <div className="text-4xl md:text-5xl mb-3 md:mb-4">{skill.icon}</div>
                  <h3 className="font-semibold text-text-primary group-hover:text-accent transition-colors text-sm md:text-base">
                    {skill.name}
                  </h3>
                </div>
              ))}
          ...
          {/* Right Side - Skill Progress Bars & Certificates */}
          <div className="space-y-8 md:space-y-12 animate-slide-in-right" style={{ animationDelay: "0.4s" }}>
            {/* Proficiency Section */}
            <div>
              <h3 className="text-2xl md:text-3xl lg:text-4xl font-bold text-text-primary mb-4 md:mb-6">
                Proficiency Levels
              </h3>
            ...
            <div className="space-y-6 max-h-96 md:max-h-[500px] overflow-y-auto pr-4">
              {skills.map((skill, index) => (
                ...
                >
                  <div className="flex justify-between items-center mb-3">
                    <span className="font-medium text-text-primary text-base md:text-lg">{skill.name}</span>
                    <span className="text-accent font-semibold text-sm md:text-base">{skill.percentage}%</span>
                  </div>
                  <div className="skill-bar h-3 md:h-4">
                    <div
            ...
            <div className="mt-12">
              <h3 className="text-2xl md:text-3xl lg:text-4xl font-bold text-text-primary mb-6 md:mb-8">
                Certificates
              </h3>
              <CertificatesSlider />
            </div>
